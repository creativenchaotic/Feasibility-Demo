...
SPHSimulationComputeShaderSecondPass::SPHSimulationComputeShaderSecondPass(ID3D11Device* device, HWND hwnd) : BaseShader(device, hwnd)
{
    initShader(L"SPHSimulationSecondPass_CS.cso", NULL);
}
...
void SPHSimulationComputeShaderSecondPass::setShaderParameters(ID3D11DeviceContext* dc)
{
    dc->CSSetUnorderedAccessViews(0, 1, &particlesOutputWritable, 0);//Same as UAVs
}


void SPHSimulationComputeShaderSecondPass::unbind(ID3D11DeviceContext* dc)
{
...
void SPHSimulationComputeShaderSecondPass::setSimulationDataSRV(ID3D11DeviceContext* deviceContext, ID3D11ShaderResourceView* sphSimulationFirstPassOutput, ID3D11ShaderResourceView* computeShaderSRVBitonicMergesortParticleIndices, ID3D11ShaderResourceView* computeShaderSRVOffsetsCalculationOutput)
{
