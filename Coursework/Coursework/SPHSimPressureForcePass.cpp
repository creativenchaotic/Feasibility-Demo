...
SPHSimPressureForcePass::SPHSimPressureForcePass(ID3D11Device* device, HWND hwnd) : BaseShader(device, hwnd)
{
    initShader(L"SPHSim_PressureForce.cso", NULL);
}
...
void SPHSimPressureForcePass::createOutputUAVs(ID3D11Device* pd3dDevice, int numParticles)//Called each time the number of particles is changed
{
...
void SPHSimPressureForcePass::setShaderParameters(ID3D11DeviceContext* dc)
{
    dc->CSSetUnorderedAccessViews(0, 1, &particlesOutputWritable, 0);//Same as UAVs
}


void SPHSimPressureForcePass::unbind(ID3D11DeviceContext* dc)
{
