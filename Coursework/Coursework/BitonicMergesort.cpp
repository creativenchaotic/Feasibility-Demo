...
void BitonicMergesort::createOutputUAVs(ID3D11Device* pd3dDevice, int numParticles, std::vector<XMFLOAT3>* particleIndices)
{
    ...
    bufferDesc.ByteWidth = numParticles * sizeof(XMFLOAT3);//sizeofT should be the particle data struct. Setting the size of the buffer to whatever amount is needed
    ...
    bufferDesc.StructureByteStride = sizeof(XMFLOAT3);//make this the size of particle data//Setting the stride that the buffer needs to take when reading each element from memory
    D3D11_SUBRESOURCE_DATA bufferInitData;
    bufferInitData.pSysMem = particleIndices->data();//Initial data that is getting passed into the buffer
    pd3dDevice->CreateBuffer(&bufferDesc, (particleIndices) ? &bufferInitData : nullptr, &bitonicMergesortOutput);//Creates the buffer
