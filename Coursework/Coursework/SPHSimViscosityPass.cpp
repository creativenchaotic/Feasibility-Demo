...
SPHSimViscosityPass::SPHSimViscosityPass(ID3D11Device* device, HWND hwnd) : BaseShader(device, hwnd)
{
    initShader(L"SPHSim_Viscosity.cso", NULL);
}
...
void SPHSimViscosityPass::createOutputUAVs(ID3D11Device* pd3dDevice, int numParticles)//Called each time the number of particles is changed
{
...
void SPHSimViscosityPass::setShaderParameters(ID3D11DeviceContext* dc)
{
    dc->CSSetUnorderedAccessViews(0, 1, &particlesOutputWritable, 0);//Same as UAVs
}


void SPHSimViscosityPass::unbind(ID3D11DeviceContext* dc)
{
